...
/*
	The data of testing£º
	plaintext:01234567 89abcdef fedcba98 76543210
	key:£º01234567 89abcdef fedcba98 76543210
	ciphertext£º681edf34 d206965e 86b3e94f 536e4246
*/
int main(void) 
{
	...
	printf("Plaintext:");
	scanf_s("%8x", &X[0], sizeof(X[0]));
	scanf_s("%8x", &X[1], sizeof(X[1]));
	scanf_s("%8x", &X[2], sizeof(X[2]));
	scanf_s("%8x", &X[3], sizeof(X[3]));
	getx(X, x8);

	printf("\nKey:");
	scanf_s("%8x", &MK[0], sizeof(MK[0]));
	scanf_s("%8x", &MK[1], sizeof(MK[1]));
	scanf_s("%8x", &MK[2], sizeof(MK[2]));
	scanf_s("%8x", &MK[3], sizeof(MK[3]));
	

	printf("\nGenerate Round Keys:\n");
	getRK(MK, K, RK);
	for (i = 0; i < 32; i++) {
		printf("[%2d]£º%08x    ", i, RK[i]);
		if (i % 4 == 3)	printf("\n");
	}
	...
	printf("\nWhite-box implementation (Please wait a moment)£º");
